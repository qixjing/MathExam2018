...
public class MathExam6352 {
	...
	private void inPut(String str0,String str1) {
		...
		while (true) {
			...
			try {
				...
				if(!flag1){
					System.out.println("输入错误，只能输入99以内的题目数量，请重新输入题目数量 : ");
					str0 = in.next();
				} 
				if(!flag2){
					System.out.println("输入错误，年级为\"1\"或\"2\" ，请重新输入年级 : ");
					str1 = in.next();
				}
				continue;
			}			
	...
	private void mul(int n1, int n2,int i) {
		...
		answer = n1 * n2;
		str_MathProblem[i] = "( " + (i+1) +" ) " + n1 + " x " + n2 + " = ";
		str_MathAnswer[i] = "( " + (i+1) +" ) " + n1 + " x " + n2 + " = " + answer;
	}
	...
	private void div(int i) {
		...
		while(true){
			...
			if(n1 % n2 == 0){
				...
				answer = n1 / n2;
				str_MathProblem[i] = "( " + (i+1) +" ) " + n1 + " / " + n2 + " = ";
				str_MathAnswer[i] = "( " + (i+1) +" ) " + n1 + " / " + n2 + " = " + answer + "..." + (n1 % n2);
			}
			break;
		}
	}
